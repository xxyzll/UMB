...
def evaluate(model, postprocessors, data_loader, base_ds, device, output_dir, args):
    ...
    torch.save({
        'lines': coco_evaluator.lines,
        'lines_cls': coco_evaluator.lines_cls,
        'att_sim': coco_evaluator.att_sim,
        'cate_sim': coco_evaluator.cate_sim,
    }, os.path.join(output_dir, f'{args.dataset}_eval.pth'))
...
def plot_bboxes_on_image(detections, img, output_dir, image_name, cls_names, num_known=10, num_unknown=5,
                         img_size=None, vis_known_label=False, vis_unknown_label=False):
    ...
    if True:
        ...
        unk_sim = None
        mean_y = None
        if detections['unknown_sim'] is not None:
            unk_sim = detections['unknown_sim'][unk_ind][indices]
            mean_y = detections['mean_y'][unk_ind][indices]
    ...
    for i in range(len(labels)):
        ...
        if (label == 'unknown' and score > -0.025) or \
            ...
            if label == 'unknown':
                ...
                if unk_sim is not None:
                    label = f'{label} {unknown_id}'
                    if unknown_id not in unknown_label_msg:
                        unknown_label_msg[unknown_id] = {}
                    unknown_label_msg[unknown_id]['unknown_sim'] = unk_sim[unknown_id - 1]
                    unknown_label_msg[unknown_id]['mean_y'] = mean_y[unknown_id - 1]
                    unknown_id += 1
            else:
            ...
            ax.add_patch(rect)
            # 不显示已知的label
            if 'unknown' not in label and (not vis_known_label):
                continue
            if 'unknown' in label and (not vis_unknown_label):
                continue
            
            ax.text(xmin, ymin - 5, f'{label}: {score:.2f}', 
                    fontsize=10, color= ('b' if 'unknown' in label else 'g'))
            
    plt.savefig(os.path.join(output_dir, 'vis', image_name), dpi=300, bbox_inches='tight', pad_inches=0)
    if len(unknown_label_msg) > 0:
        torch.save(unknown_label_msg, os.path.join(output_dir, 'vis', image_name.replace('.jpg', '.pth')))
    return
